...
import uz.pevops.repository.ProductRepository;
import uz.pevops.service.criteria.ProductCriteria;
import uz.pevops.service.dto.ProductDTO;
import uz.pevops.service.mapper.ProductMapper;
...
class ProductResourceIT {
    ...
    public static Product createEntity(EntityManager em) {
        Product product = new Product().name(DEFAULT_NAME).status(DEFAULT_STATUS).code(DEFAULT_CODE);
        return product;
    }
    ...
    public static Product createUpdatedEntity(EntityManager em) {
        Product product = new Product().name(UPDATED_NAME).status(UPDATED_STATUS).code(UPDATED_CODE);
        return product;
    }
    ...
    @Test
    @Transactional
    void getAllProductsByPhotoIsEqualToSomething() throws Exception {
        Attachment photo;
        if (TestUtil.findAll(em, Attachment.class).isEmpty()) {
            productRepository.saveAndFlush(product);
            photo = AttachmentResourceIT.createEntity(em);
        } else {
            photo = TestUtil.findAll(em, Attachment.class).get(0);
        }
        em.persist(photo);
        em.flush();
        ...
        // Get all the productList where photo equals to photoId
        defaultProductShouldBeFound("photoId.equals=" + photoId);

        // Get all the productList where photo equals to (photoId + 1)
        defaultProductShouldNotBeFound("photoId.equals=" + (photoId + 1));
    }

    @Test
    @Transactional
    void getAllProductsByMeasurementIsEqualToSomething() throws Exception {
        Measurement measurement;
        if (TestUtil.findAll(em, Measurement.class).isEmpty()) {
            productRepository.saveAndFlush(product);
            measurement = MeasurementResourceIT.createEntity(em);
        } else {
            measurement = TestUtil.findAll(em, Measurement.class).get(0);
        }
        ...
        // Get all the productList where measurement equals to measurementId
        defaultProductShouldBeFound("measurementId.equals=" + measurementId);

        // Get all the productList where measurement equals to (measurementId + 1)
        defaultProductShouldNotBeFound("measurementId.equals=" + (measurementId + 1));
    }
    ...
    void putNewProduct() throws Exception {
        ...
        updatedProduct.name(UPDATED_NAME).status(UPDATED_STATUS).code(UPDATED_CODE);
    ...
    void fullUpdateProductWithPatch() throws Exception {
        ...
        partialUpdatedProduct.name(UPDATED_NAME).status(UPDATED_STATUS).code(UPDATED_CODE);
